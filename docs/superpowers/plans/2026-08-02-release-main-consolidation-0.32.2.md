...
- [x] Validate every modified JSON file with `ConvertFrom-Json` and inspect `git diff --check`.
- [x] Commit the release metadata before merging branch histories.
...
- [x] Confirm `codex/canvas-growth-zoom-minimap-tags` is clean and `main` equals `origin/main`.
- [x] Fast-forward `main` to `codex/canvas-growth-zoom-minimap-tags`.
- [x] Merge `origin/dev`, `origin/feat/agent-permission-policy` and `origin/feat/canvas-workspace` with `git merge --no-ff -s ours --no-edit <branch>`.
- [x] Confirm `origin/release`, `origin/foundation/notegen-636d4f8`, `upstream/dev` and the local foundation branch are already ancestors or safely preserved without editing their worktree.
- [x] Run `git log main..<branch>` for every named branch and verify zero remaining unique commits.
...
- [x] Run the complete Canvas suite, `node node_modules/typescript/bin/tsc --noEmit`, the locked Next.js production build, and `cargo check --manifest-path src-tauri/Cargo.toml --locked`.
- [x] Build only NSIS with `node_modules/.bin/tauri.CMD build --bundles nsis`.
- [x] Locate the generated installer with `Get-ChildItem src-tauri/target/release/bundle/nsis -Filter *.exe` and compute SHA-256 with `Get-FileHash`.
- [x] Start the installer using `Start-Process -Wait -WindowStyle Hidden -ArgumentList '/S'` and require exit code 0.
