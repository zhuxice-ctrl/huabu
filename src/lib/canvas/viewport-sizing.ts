...
export function normalizeCanvasZoom(value: unknown, fallback = INITIAL_CANVAS_ZOOM): number {
...
function snapshotZoom(snapshot: ViewportSnapshot): number {
  return normalizeCanvasZoom(snapshot.zoom)
}
...
export function captureViewportSnapshot(input: {
  ...
  return Object.freeze({
    ...
    zoom: normalizeCanvasZoom(viewport.zoom, lastValid?.zoom),
...
export function contentScaleForZoom(zoom: number): number {
  return clamp(round4(1 / normalizeCanvasZoom(zoom)), MIN_CONTENT_SCALE, MAX_CONTENT_SCALE)
}

export function resolveZoomAwareTextDrawSize(size: CanvasSize, zoom: number): CanvasSize {
  const effectiveZoom = Math.sqrt(Math.max(1, normalizeCanvasZoom(zoom)))
  return {
